...
export default function AnnotationControls({
    ...
    const loadQuestions = async () => {
      try {
        ...
        const answerGroups = await Promise.all(
          segments.filter(segment => Number(segment.id) > 0)
            .map(segment => getAnswersForSegment(Number(segment.id)))
        );
        if (!cancelled) {
          setQuestions(active);
        ...
        if (!cancelled) setQuestionError(error instanceof Error ? error.message : "Unable to load questions.");
      }
    };
    void loadQuestions();
  ...
  const saveQuestionAnswer = async (question: QuestionDto) => {
    ...
    try {
      ...
      setQuestionError(error instanceof Error ? error.message : "Unable to save answer.");
    }
  };
