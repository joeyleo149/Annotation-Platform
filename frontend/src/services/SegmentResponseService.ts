...
export async function createSegment(request: SegmentResponseRequest) { return parseResponse<SegmentResponseDto>(await fetch(endpoint("/"), { method: "POST", headers: requestHeaders(), body: JSON.stringify(withSubmittedAt(request)) })); }
export async function updateSegment(id: number, request: SegmentResponseRequest) { return parseResponse<SegmentResponseDto>(await fetch(endpoint(`/${id}`), { method: "PUT", headers: requestHeaders(), body: JSON.stringify(withSubmittedAt(request)) })); }
export async function deleteSegment(id: number) { const response = await fetch(endpoint(`/${id}`), { method: "DELETE", headers: requestHeaders() }); if (!response.ok) await parseResponse<never>(response); }
export function secondsToTimeSpan(totalSeconds: number) { const safe = Math.max(0, totalSeconds); const h = Math.floor(safe / 3600); const m = Math.floor((safe % 3600) / 60); const s = safe % 60; return `${String(h).padStart(2, "0")}:${String(m).padStart(2, "0")}:${s.toFixed(3).padStart(6, "0")}`; }
export function timeSpanToSeconds(value: string) { const parts = value.split(":").map(Number); return parts.length === 3 && !parts.some(Number.isNaN) ? parts[0] * 3600 + parts[1] * 60 + parts[2] : 0; }
