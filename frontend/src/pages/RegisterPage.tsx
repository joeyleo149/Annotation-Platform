 ...
 const [showPassword, setShowPassword] = useState(false);
  ...
  return <AuthLayout><div className="auth-icon" aria-hidden="true"><UserPlus /></div><h1>Create Your Account</h1><p className="auth-subtitle">Join Annotate Pro and start annotating videos<br />with ease.</p><div className="auth-divider" />
    <form onSubmit={submit} className="auth-form compact">
      ...
      <Field label="Password"><div className="password-field"><input type={showPassword ? 'text' : 'password'} value={form.password} onChange={set('password')} placeholder="Enter your password" minLength={8} required /><button type="button" onClick={() => setShowPassword(v => !v)} aria-label={showPassword ? 'Hide password' : 'Show password'}>{showPassword ? <EyeOff /> : <Eye />}</button></div></Field>
